...
vm_alloc_page_with_initializer (enum vm_type type, void *upage, bool writable,
	...
	//printf("vm_alloc_page_with_init은 들어가나\n"); // 왜 처음에 5번이나 들어가지?
	// 5번->2번->1번->그 뒤 안 들어가다가 에러 발생
	//printf("spt: 0x%x\n", spt);
...
spt_insert_page (struct supplemental_page_table *spt UNUSED,
	...
	*/
	bool succ = false;
	//printf("spt_insert_page까지는 잘 와?\n");
	//printf("spt addr: 0x%x\n", spt);
	//printf("spt page table addr: 0x%x\n", spt->page_table);
	//printf("page: 0x%x\n", page);
	/* TODO: Fill this function. */
...
vm_try_handle_fault (struct intr_frame *f UNUSED, void *addr UNUSED,
	...
	/* TODO: Your code goes here */
	//printf("(handle fault 시작할 떄) f->rsp: 0x%x\n", f->rsp);
	...
	if (is_kernel_vaddr(addr) && user || addr == NULL || not_present == false) {
		...
		if (page == NULL) {
			...
			if (write && page->write == false) {
				...
				//NOT_REACHED();
				//printf("여기 몇 번 들어가는 거야?\n");
				//printf("page va: 0x%x\n", page->va);
				//printf("handle fault thread current for if rsp: 0x%x\n", &thread_current()->if_for_fork.rsp);
				//printf("handle fault f->rsp: 0x%x\n", f->rsp);
				//f->rsp = 0x4747fff6;
				//printf("after handle fault f->rsp: 0x%x\n", f->rsp);
...
vm_do_claim_page (struct page *page) {
	...
	//printf("(before swap) frame va: 0x%x\n", frame->kva);
	//printf("(before swap) frame->page->va: 0x%x\n", frame->page->va);
	if (mapping_va_pa) {
		//printf("제발\n");
		bool success_swapping = swap_in (page, frame->kva);
		//printf("(after swap) frame kva: 0x%x\n", frame->kva);
		//printf("(after swap) frame->page->va: 0x%x\n", frame->page->va);
		//printf("pml4: 0x%x\nS", &thread_current()->pml4);
		return success_swapping;
	} else {
		return false;
	}
	/*
...
void
spt_destroy (struct hash_elem *elem, void *aux UNUSED) {
	vm_dealloc_page (hash_entry(elem, struct page, hash_elem));
}