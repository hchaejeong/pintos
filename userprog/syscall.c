...
remove (const char *file) {
	if (file == NULL) {
		return false; // 당연히 file이 null이면 return
	}
	check_address(file);
...
open (const char * file) {
	...
	struct fd_structure *fd_elem = palloc_get_page(0);
	...
	if (!actual_file || actual_file == NULL) {		//파일을 열지 못한 경우
	...
	if (fd_elem != NULL) {
		...
		if (!strcmp(file, thread_current()->name)) {
			// 현재 thread와 파일 이름이 다르면 이 thread는 파일을 건들 수 없다!
			file_deny_write(actual_file);
		}
		//lock_release(&file_lock);
	} else {
		//lock_release(&file_lock);
...
fork (const char *thread_name, struct intr_frame *f) {
	//printf("여긴 들어가?\n");
	//lock_acquire(&file_lock);
	tid_t fork = process_fork(thread_name, f);
	//lock_release(&file_lock);
	return fork;
}
...
exec (const char *file) {
	...
	if (file_name == NULL) {
		...
		//lock_acquire(&file_lock); // process_exec에는 load가 있으므로 lock 걸어줘야
		result = process_exec(file_name);
		//lock_release(&file_lock);
		...
		thread_current()->exit_num = result;
		return result;
	}
	// exit(-1);
